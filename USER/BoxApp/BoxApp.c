...
void SonarApp()
{
	...
	//////////////////
	if (BoxIO.TimeStamp - 1 > oldTimeStampForCnt || oldTimeStampForCnt > BoxIO.TimeStamp)
	{ // every 2 ms exeed;
		NodenDis[pNowNode][2]++;
		oldTimeStampForCnt = BoxIO.TimeStamp;
	}
	if (BoxIO.TimeStamp - 1000 > 0 || oldTimeStampFor1sCnt > BoxIO.TimeStamp)
	{ // every 1 s exeed;
	...
	if (BoxIO.TimeStamp - 3 > oldTimeStampFor4msCnt || oldTimeStampFor4msCnt > BoxIO.TimeStamp)
	{ // every 4 ms exeed;
		//////////////////////////2 3 2////////////////////////////
	...
	if (BoxIO.TimeStamp - 40 > oldTimeStampFor25msCnt || oldTimeStampFor25msCnt > BoxIO.TimeStamp)
	{
	...
	else if (BoxIO.Uart2Dat.pRfil == 2)
	{
	...
	else
	{
		...
		timesExit[pNowNode] += BoxIO.TimeStamp > oldTimeStampForAlive ? BoxIO.TimeStamp - oldTimeStampForAlive : 0;
		if (timesExit[0] > 300)
		{
	...
	////////////Data flow
	if (HandShakeCap(&BoxIO.Uart3Dat))
	{ // RS232
		SonarDual.cmdPCConnect = 1;
		SonarDual.ReqConnectAllow = 1;
	}
	if (PCChangeParCap(&BoxIO.Uart3Dat))
	{ // RS232
		gSystemPara.cmdSaveParameter = 1;
	}
	if (PCCalSensor(&BoxIO.Uart3Dat))
	{ // RS232
	...
	if (ChangeFilterCap(&BoxIO.Uart3Dat))
	{ // RS232
		BoxIO.Uart2Dat.sBuffer[0] = SonarDual.ActID;
		BoxIO.Uart2Dat.sBuffer[1] = 0x02;
